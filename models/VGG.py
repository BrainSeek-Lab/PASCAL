...
class VGG(nn.Module):
    def __init__(self, vgg_name, num_classes, dropout):
        ...
        self.loss = 0 
        self.list = [8,4,4,1,4,4,1,1,1,1,1,4,8,8,8]
        self.merge = MergeTemporalDim(self.list[-1])
        self.expand = ExpandTemporalDim(self.list[-1])
        self.index = 0
    ...
    def _make_layers(self, cfg, dropout):
        ...
        for x in cfg:
            if x == 'M':
                ...
                layers.append(IF(self.index,self.list))
